...
//                             Resolve.isStatic(env));//DEBUG
    ...
    public void visitTypeParameter(JCTypeParameter tree) {
        ...
        if (tree.bounds.nonEmpty()) {
            ...
            if (b.tag == TYPEVAR) {
                ...
                for (JCExpression bound : tree.bounds.tail) {
                    Type i = attribBase(bound, env, false, true, false);
                    if (i.tag == CLASS) {
                        boundBuf.append(i);
                        chk.checkNotRepeated(bound.pos(), i, boundSet);
                    }
                }
            }
        }
        ...
        types.setBounds(a, bs.nonEmpty() ? bs : List.of(syms.objectType));
    ...
    public void visitErroneous(JCErroneous tree) {
        if (tree.errs != null)
            for (List<JCTree> l = tree.errs; l.nonEmpty(); l = l.tail)
                attribTree(l.head, env, ERR, pt);
        result = tree.type = syms.errType;
    }
